...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .setDescription('Performs an unwhitelist action on the selected user.')
        ...
        try {
          ...
          if (success >= total / 2) {
            const embed = new EmbedBuilder()
              ...
              .setDescription(`\`🟢\` \`System Success\`\nThe selected user has been unwhitelisted!\nUnwhitelisted on \`'${success}'\` of \`'${total}'\` servers.\nIdentifier: \`'${username}'\``);
            ...
            const embed = new EmbedBuilder()
              ...
              .setTitle('`Obelisk Management`')
              .setFooter({ text: `Tip: Contact support if there are issues.\nPerformance response: ${Math.trunc(duration)}ms.` })
              .setDescription(`\`🟠\` \`System Failure\`\nThe selected user has not been unwhitelisted!\nUnwitelisted on \`'${success}'\` of \`'${total}'\` servers.\nIdentifier: \`'${username}'\``);
